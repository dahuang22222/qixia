...
//         result[1]=y;
    ...
    public static double GetDistance(BigDecimal lon1, BigDecimal lat1, BigDecimal lon2, BigDecimal lat2) {
        double radLat1 = rad(Double.valueOf(lat1.toString()));
        double radLat2 = rad(Double.valueOf(lat2.toString()));
        double a = radLat1 - radLat2;
        double b = rad(Double.valueOf(lon1.toString())) - rad(Double.valueOf(lon2.toString()));
        double s = 2 * Math.asin(Math.sqrt(
                Math.pow(Math.sin(a / 2), 2) + Math.cos(radLat1) * Math.cos(radLat2) * Math.pow(Math.sin(b / 2), 2)));
        s = s * EARTH_RADIUS;
        // s = Math.round(s * 10000) / 10000;
